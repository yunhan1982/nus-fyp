...
async def run_delta_query(solution, name: str, vt_from: datetime, vt_to: datetime, tt_from: datetime, verbose: bool = False):
    start_time = time.perf_counter()
    async with timer(f"{solution.name} Range Query") as t:
        records = await solution.delta_since_vt_range(name, vt_from, vt_to, tt_from)
...
async def main():
    ...
    # List of input sets - each tuple contains (name, vt_from, vt_to, tt_from)
    input_sets = [
        # Input Set 1
        ("Student_4178", 
         datetime(2022, 3, 29, 0, 0, 0, tzinfo=timezone.utc),  # vt_from
         datetime(2022, 4, 7, 0, 0, 0, tzinfo=timezone.utc),  # vt_to
         datetime(2022, 4, 2, 0, 0, 0, tzinfo=timezone.utc),  # tt_from
         ), 
        ...
        ("Student_1741",
         datetime(2018, 1, 1, 0, 0, 0, tzinfo=timezone.utc),   # vt_from
         datetime(2018, 1, 30, 0, 0, 0, tzinfo=timezone.utc),  # vt_to
         datetime(2018, 1, 8, 0, 0, 0, tzinfo=timezone.utc),   # tt_from
        )
    ]
    ...
    for solution in solutions:
        ...
        for i, (name, vt_from, vt_to, tt_from) in enumerate(input_sets):
            print(f"Running {solution.name} with Input Set {i+1}...")
            records, runtime_ms = await run_delta_query(solution, name, vt_from, vt_to, tt_from, verbose=False)
            solution_runtimes.append(f"{runtime_ms:.3f}")
            print(f"Found records in {runtime_ms:.3f}ms")
            print(records)
    ...
    print("\n=== Input Set Details ===")
    for i, (name, vt_from, vt_to, tt_from) in enumerate(input_sets):
        print(f"Input Set {i+1}: name='{name}'")
        print(f"  Valid Time: {vt_from.strftime('%Y-%m-%d')} to {vt_to.strftime('%Y-%m-%d')}")
        print(f"  Transaction Time: {tt_from.strftime('%Y-%m-%d')}")
