...
async def run_delta_query(solution, name: str, vt_from: datetime, tt_from: datetime, tt_to: datetime, verbose: bool = False):
    start_time = time.perf_counter()
    async with timer(f"{solution.name} Range Query") as t:
        records = await solution.delta_since_tt_range(name, tt_from, tt_to, vt_from)
...
async def main():
    ...
    # List of input sets - each tuple contains (name, vt_from, vt_to, tt_from, tt_to)
    input_sets = [
        # Input Set 1
        ("Student_1669", 
    ...
    for solution in solutions:
        ...
        for i, (name, vt_from, vt_to, tt_from, tt_to) in enumerate(input_sets):
            print(f"Running {solution.name} with Input Set {i+1}...")
            records, runtime_ms = await run_delta_query(solution, name, vt_from, vt_to, tt_from, tt_to, verbose=False)
